...
export default function SentenceManager({ sentences, onRefresh }) {
  ...
  return (
    <div className="p-4 sm:p-6">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Sentence Management</h2>
      
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
        <h3 className="text-base sm:text-lg font-semibold mb-4">Add New Sentence</h3>
        ...
        {showSuccess && <div className="mt-4 text-green-600 flex items-center gap-2"><Check size={20} /> Sentence added successfully!</div>}
      </div>

      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
        <h3 className="text-base sm:text-lg font-semibold mb-4">All Sentences ({sentences.length})</h3>
        ...
        <div className="max-h-96 overflow-y-auto space-y-3">
          {filteredSentences.length === 0 ? (
            ...
            filteredSentences.map((sentence) => (
              <div key={sentence.id} className="bg-gray-50 rounded-lg p-4 flex justify-between items-start">
                <div>
                  <div className="font-medium mb-1">{sentence.japanese}</div>
                  <div className="text-gray-600 text-sm">{sentence.english}</div>
                </div>
                <button onClick={() => handleDelete(sentence.id)} className="text-red-500 hover:text-red-700 ml-2 p-2 flex-shrink-0">
                  <X size={20} />
                </button>
              </div>
            ))
